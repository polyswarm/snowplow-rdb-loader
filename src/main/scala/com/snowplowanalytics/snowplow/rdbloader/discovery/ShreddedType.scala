...
import com.snowplowanalytics.snowplow.rdbloader.dsl.{Cache, S3I}
...
object ShreddedType {
   ...
   */
  def discoverJsonPath[F[_]: Monad: Cache: S3I](env: Environment[F], region: String, jsonpathAssets: Option[S3.Folder], shreddedType: Info): DiscoveryAction[F, S3.Key] = {
      ...
      case None =>
        jsonpathAssets match {
          case Some(assets) =>
            ...
            S3I[F].keyExists(env, s3Key).flatMap {
              case true =>
                for {
                  _ <- Cache[F].putCache(key, Some(s3Key))
                } yield s3Key.asRight
              case false =>
                getSnowplowJsonPath[F](env, region, key)
            }
          case None =>
            getSnowplowJsonPath[F](env, region, key)
        }
   ...
   */
  def getSnowplowJsonPath[F[_]: Monad: S3I: Cache](env: Environment[F],
                                                   s3Region: String,
  ...
  def discoverBatch[F[_]: Monad: Cache: S3I](env: Environment[F],
                                             region: String,
    ...
    def discover(info: ShreddedType.Info): F[ValidatedNel[DiscoveryFailure, ShreddedType]] = {
      val jsonpaths: F[DiscoveryStep[S3.Key]] = ShreddedType.discoverJsonPath[F](env, region, jsonpathAssets, info)
