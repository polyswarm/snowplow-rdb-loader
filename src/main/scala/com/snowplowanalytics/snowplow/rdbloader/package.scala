...
package object rdbloader {
  ...
  object LoaderAction {
    ...
    def lift[F[_]: Applicative, A](value: A): LoaderAction[F, A] =
    ...
    def liftA[F[_]: Functor, A](action: F[A]): LoaderAction[F, A] =
  ...
  implicit class ActionOps[F[_], A](a: F[A]) {
    def liftA(implicit F: Functor[F]): LoaderAction[F, A] =
      LoaderAction.liftA(a)
  }
